...
double SIMD_test(data_t *l_tax, data_t *l_extendedprice, data_t *l_quantity,int columnlength,int vector_len)
{
    ...
    data_t *tax_value=(data_t *)malloc(sizeof(data_t) * vector_len);
    data_t *total_value=(data_t *)malloc(sizeof(data_t) * vector_len);
    size_t nBlockWidth = 32.0/(sizeof(float));    //���. AVX�Ĵ�����һ�δ���data_t�������ݵ�����.
    size_t cntBlock = vector_len / nBlockWidth;    //������SIMD����.
    size_t cntRem = vector_len % nBlockWidth;    //ʣ������.
    __m256 yfsLoad1;    //����extendedprice����.
    __m256 yfsLoad2;    //����quantity����.
    __m256 yfsLoad3;    //����tax����.
    __m256 yfsLoad4;    //����1+tax����.
    const data_t* p1 = l_extendedprice;    // AVX��������ʱ���õ�ָ��.
    const data_t* p2 = l_quantity;    // AVX��������ʱ���õ�ָ��.
    const data_t* p3 = l_tax;    // AVX��������ʱ���õ�ָ��.
    data_t* p4 = netto_value;    // AVX��������ʱ���õ�ָ��.
    data_t* p5 = tax_value;    // AVX��������ʱ���õ�ָ��.
    data_t* p6 = total_value;    // AVX��������ʱ���õ�ָ��.
    
    __m256 yfsSum1 = _mm256_setzero_ps();    //l_extendedprice*l_quantity��ʱ���������[AVX] ����ֵ0
    __m256 yfsSum2 = _mm256_setzero_ps();    //1-l_tax��ʱ���������[AVX] ����ֵ0
    __m256 yfsSum_plus_1 = _mm256_set1_ps(1);  // [AVX] ��������1
    __m256 yfsSum3 = _mm256_setzero_ps();    //�ۼ�������������[AVX] ����ֵ0
    data_t* q = (data_t*)malloc(nBlockWidth * sizeof(data_t));    //��AVX�����ϵĶ����ֵ�ϲ�ʱ����ָ��.
//    const data_t* q;
    ...
    for (i = 0; i < columnlength/vector_len; i++) {  /*loop with vector granularity*/
        ...
        if(vector_len<nBlockWidth) {
            ...
            for(m=0; m<cntBlock; ++m){
                ...
                yfsLoad1 = _mm256_loadu_ps(p1);    // [AVX] ����l_extendedprice
                yfsLoad2 = _mm256_loadu_ps(p2);    // [AVX] ����l_quantity
                yfsSum1 = _mm256_mul_ps(yfsLoad1, yfsLoad2);// [AVX] ������������ӷ�
                //            _mm256_store_ps(p4,yfsSum1);
                yfsLoad3 = _mm256_loadu_ps(p3);   // [AVX] ����l_tax
                yfsLoad4 = _mm256_add_ps(yfsSum_plus_1, yfsLoad3);// [AVX] ������������ӷ�ִ��1+l_tax
                //            _mm256_store_ps(p5,yfsLoad4);
                //             yfsSum2 = _mm256_mul_ps(_mm256_load_ps(p4), _mm256_load_ps(p5));// [AVX] ������������ӷ�
                yfsSum2 = _mm256_mul_ps(yfsSum1, yfsLoad4);// [AVX] ������������ӷ�
